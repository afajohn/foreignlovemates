...
import type { trashArticle, trashArticleFormProps} from "@/app/types/trash";
import TiptapEditor from "./TipTapComponent";
import { useTrashArticleForm } from "@/hooks/trash";

export default function trashArticleForm({ articlesData, editId}: trashArticleFormProps) {
  const {
  ...
  } = useTrashArticleForm(articlesData as object as trashArticle);
  
  return (
    <div> 
      <form action="submit">
        <div className="flex">
          <h1 className="text-xl font-bold mb-4">{editId ? "Edit trashArticle" : "Publish trashArticle"}</h1>
